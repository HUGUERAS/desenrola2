...
function g(n2, o, { z: u2, m: l4 }, f2, c2) {
  ...
  const [p3, v3] = n2;
  return c2 ? i(c2, p3, v3, d4, s2) : c2 = t(p3, v3, d4, s2), null != l4 && (c2.m = l4, c2.hasM = true), c2;
}
...
function y2(t3, r3, s2, o, c2 = s2) {
  return e(q2, o, s2), e(k, r3, c2), l3(w, k, q2), u(t3, c2, w);
}
...
function M(t3, e2) {
  const s2 = t3.start, o = t3.end, c2 = e2.start, u2 = e2.end, i2 = e(q2, o, s2), f2 = e(U2, u2, c2), a2 = m4(i2, f2);
  if (Math.abs(a2) <= v2) return [];
  const p3 = e(k, s2, c2), h2 = m4(f2, p3) / a2, l4 = m4(i2, p3) / a2;
  if (h2 >= 0) {
    if (l4 >= 0 || 1 === e2.type) return [d(w, s2, i2, h2)];
  } else if (1 === t3.type && (l4 >= 0 || 1 === e2.type)) return [d(w, s2, i2, h2)];
  return [];
}
function g2(t3, e2, s2) {
  const u2 = [], i2 = e(q2, t3.end, t3.start), f2 = e(U2, t3.start, e2), a2 = p(i2), p3 = 2 * j(i2, f2), m5 = p3 * p3 - 4 * a2 * (p(f2) - s2 * s2);
  if (0 === m5) {
    const n2 = -p3 / (2 * a2);
    (1 === t3.type || n2 >= 0) && u2.push(d(w, t3.start, i2, n2));
  } else if (m5 > 0) {
    const n2 = Math.sqrt(m5), e3 = (-p3 + n2) / (2 * a2);
    (1 === t3.type || e3 >= 0) && u2.push(d(w, t3.start, i2, e3));
    const s3 = (-p3 - n2) / (2 * a2);
    (1 === t3.type || s3 >= 0) && u2.push(d(k, t3.start, i2, s3));
  }
  return u2;
}
var v2 = 1e-6;
...
//# sourceMappingURL=chunk-DLFEC5DU.js.map
