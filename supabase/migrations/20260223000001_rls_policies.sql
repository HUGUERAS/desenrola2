...
CREATE POLICY "projetos: acesso por tenant" ON projetos
  FOR ALL USING (
    tenant_id = (
      SELECT tenant_id FROM perfis WHERE user_id = auth.uid()
    )
    OR
    EXISTS (
      SELECT 1 FROM lotes
...
CREATE POLICY "lotes: proprietario por email" ON lotes
  FOR SELECT USING (email_cliente = auth.email());

CREATE POLICY "lotes: proprietario atualiza proprio" ON lotes
  FOR UPDATE USING (email_cliente = auth.email());
...
DROP POLICY IF EXISTS "topografos_all_documentos" ON documentos;
