...
-- ─── projetos ─────────────────────────────────
DROP POLICY IF EXISTS "projetos: acesso por tenant" ON projetos;
...
-- ─── lotes ────────────────────────────────────
DROP POLICY IF EXISTS "lotes: topografo do tenant" ON lotes;
DROP POLICY IF EXISTS "lotes: proprietario por email" ON lotes;
DROP POLICY IF EXISTS "lotes: proprietario atualiza proprio" ON lotes;
...
-- ─── confrontacoes ────────────────────────────
DROP POLICY IF EXISTS "confrontacoes: via lote acessível" ON confrontacoes;
...
-- ─── orcamentos ───────────────────────────────
DROP POLICY IF EXISTS "orcamentos: via projeto do tenant" ON orcamentos;
...
-- ─── despesas ─────────────────────────────────
DROP POLICY IF EXISTS "despesas: via projeto do tenant" ON despesas;
...
-- ─── pagamentos ───────────────────────────────
DROP POLICY IF EXISTS "pagamentos: via lote acessível" ON pagamentos;
...
-- ─── documentos ───────────────────────────────
DROP POLICY IF EXISTS "documentos: via lote acessível" ON documentos;
...
CREATE POLICY "documentos: proprietario" ON documentos
  ...
  USING (
    lote_id::bigint IN (
      SELECT id FROM lotes WHERE email_cliente = auth.email()
    )
  );
